...
pub async fn notes_list_handler(opts: FilterOptions, db: DB) -> Result<impl Reply, Rejection> {
...
pub async fn create_note_handler(body: CreateNoteSchema, db: DB) -> Result<impl Reply, Rejection> {
    ...
    Ok(with_status(reply::json(&note), StatusCode::CREATED))
}
...
pub async fn get_note_handler(id: String, db: DB) -> Result<impl Reply, Rejection> {
    ...
    Ok(with_status(reply::json(&note), StatusCode::OK))
}
...
/// or a JSON response with a 'not found' status if the note does not exist, or a rejection if an error occurs.
pub async fn delete_note_handler(id: String, db: DB) -> Result<impl Reply, Rejection> {
    ...
    Ok(with_status(reply::json(&""), StatusCode::NO_CONTENT))
}
